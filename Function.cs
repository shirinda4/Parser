...
namespace SimpleCompiler
{
    public class Function : JackProgramElement
    {
        ...
        public override void Parse(TokensStack sTokens)
        {
            ...
            if (Body.Count>0)
            {
                if (!(Body.Last() is ReturnStatement))
                     throw new SyntaxErrorException("Expected received: return statment", null);
                else
                {
                    Return = (ReturnStatement)Body.Last();
            ...
            {
                throw new SyntaxErrorException("syntax error", null);
            }
